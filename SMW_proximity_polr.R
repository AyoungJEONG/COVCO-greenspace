...
# base model
model1.polr <- satisf_now_fac ~ forest_route + park_route + agri_route + ndvi_buff300 +
...
write.csv(sum1.polr[,-3], 'summary_polr_greenspace_route.csv', quote = F)

# interaction with household income
model4.polr <- satisf_now_fac ~ (forest_route + park_route + agri_route) * hh_income + ndvi_buff300 +
...
write.csv(sum4.polr[,-3], 'summary_polr_greenspace_route_hh_income.csv', quote = F)

# interaction with financial worries
model5.polr <- satisf_now_fac ~ (forest_route + park_route + agri_route) * finwor_3cat + ndvi_buff300 +
...
write.csv(sum5.polr[,-3], 'summary_polr_greenspace_route_finwor.csv', quote = F)

# interaction with agecat
model8.polr <- satisf_now_fac ~ (forest_route + park_route + agri_route) * agecat + ndvi_buff300 +
...
write.csv(sum8.polr[,-3], 'summary_polr_greenspace_route_agecat.csv', quote = F)
