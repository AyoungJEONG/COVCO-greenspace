...
# base model
model1.polr <- satisf_now_fac ~ forest_buff300 + park_buff300 + agri_buff300 +
...
write.csv(sum1.polr[,-3], 'summary_polr_greenspace_areaprop.csv', quote = F)

# interaction with household income
model4.polr <- satisf_now_fac ~ (forest_buff300 + park_buff300 + agri_buff300) * hh_income +
...
write.csv(sum4.polr[,-3], 'summary_polr_greenspace_areaprop_hh_income.csv', quote = F)

# interaction with financial worries
model5.polr <- satisf_now_fac ~ (forest_buff300 + park_buff300 + agri_buff300) * finwor_3cat +
...
write.csv(sum5.polr[,-3], 'summary_polr_greenspace_areaprop_finwor.csv', quote = F)

# interaction with agecat
model8.polr <- satisf_now_fac ~ (forest_buff300 + park_buff300 + agri_buff300) * agecat +
...
write.csv(sum8.polr[,-3], 'summary_polr_greenspace_areaprop_agecat.csv', quote = F)
